...
from agent.buffers import BufferManager, AudioChunk
from agent.audio import VADEngine, STTEngine
...
class InputBrain(Brain):
    ...
    async def handle_audio_frame(self, data: bytes) -> None:
        ...
        try:
            ...
            if vad_result and not self.is_listening:
                ...
                await self.publish_event(EventType.VAD_START, {"timestamp": time.time()})
    ...
    async def _run_stt(self, audio_data: bytes) -> None:
        ...
        await self.publish_event(EventType.TRANSCRIPTION_START, {
        ...
        try:
            ...
            await self.publish_event(EventType.TRANSCRIPTION_COMPLETE, {
